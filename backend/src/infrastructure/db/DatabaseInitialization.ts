...
export class DatabaseInitialization {
  constructor(@inject(InfrastructureDiType.PostgresDatabaseConfiguration) private readonly _databaseConfiguration: PostgresDatabaseConfiguration) {}

  public init() {
    const initOptions: IInitOptions<DBExtensions> = {
      query: event => {
        console.log('EXECUTED SQL QUERY:', event.query);
      },
      error: (error, event) => {
